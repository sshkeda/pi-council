...
import { loadConfig, resolveModels, createRun, getRunsDir, getLatestFile, type RunMeta } from "./core/config.js";
import { spawnWorker, agentPaths, parseStream } from "./core/runner.js";
import { loadMeta, refreshWorker, refreshRun, isAgentDone, killPid, type WorkerState } from "./core/state.js";
import { CouncilSession } from "./core/session.js";

// --- Formatting ---
const isColor = !("NO_COLOR" in process.env) && (("FORCE_COLOR" in process.env) || process.stdout.isTTY === true || process.stderr.isTTY === true);
...
export function parseArgs(argv: string[]): { command: string; runId?: string; models?: string[]; cwd?: string; timeout?: number; prompt: string } {
  ...
  let models: string[] | undefined, cwd: string | undefined, timeout: number | undefined, runId: string | undefined, command: string | undefined;
  const rest: string[] = [];
  let i = 0;
  while (i < args.length) {
    if (args[i] === "--models" && i + 1 < args.length) { models = args[i + 1].split(",").filter(Boolean); i += 2; }
    else if (args[i] === "--cwd" && i + 1 < args.length) { cwd = args[i + 1]; i += 2; }
    else if (args[i] === "--timeout" && i + 1 < args.length) { const p = parseInt(args[i + 1], 10); if (Number.isFinite(p) && p >= 0) timeout = p; i += 2; }
    else if (args[i] === "--run-id" && i + 1 < args.length) { runId = args[i + 1]; i += 2; }
    else if (args[i] === "--") { i++; while (i < args.length) { rest.push(args[i]); i++; } }
    else if (args[i] === "--help" || args[i] === "-h") { command = "help"; i++; }
    else if (args[i] === "--version" || args[i] === "-v") { command = "version"; i++; }
    else if (!command && rest.length === 0 && KNOWN_COMMANDS.has(args[i])) {
      const cmd = args[i], next = args[i + 1];
      const isPromptCmd = cmd === "ask" || cmd === "spawn";
      const nextOk = !next || next.startsWith("--") || /^\d{8}-/.test(next);
      if (isPromptCmd || nextOk) { command = cmd; i++; } else { rest.push(args[i]); i++; }
    }
    else { rest.push(args[i]); i++; }
  }
  if (!command) command = rest.length > 0 ? "ask" : "help";
  const prompt = rest.join(" ");
  if (!runId && ["status", "results", "cleanup", "cancel", "watch"].includes(command) && rest.length > 0 && /^\d{8}-/.test(rest[0])) runId = rest[0];
  return { command, runId, models, cwd, timeout, prompt };
}

function resolveRunId(runId?: string): string {
  let resolved: string;
  if (runId) { resolved = runId; } else {
    try { resolved = fs.readFileSync(getLatestFile(), "utf-8").trim(); } catch { throw new Error("No run specified and no latest run found."); }
  }
  if (resolved.includes("..") || resolved.includes("/") || resolved.includes("\\")) throw new Error(`Invalid run ID: ${resolved}`);
  return resolved;
}
...
async function ask(prompt: string, opts: { models?: string[]; cwd?: string; timeout?: number }): Promise<void> {
  ...
  const session = new CouncilSession({
    runId, runDir, prompt, models, config, cwd: opts.cwd ?? process.cwd(),
    timeoutSeconds: opts.timeout,
    events: {
      onSpawned(m, pid) { process.stderr.write(`  🚀 ${m.id.padEnd(8)} spawned (PID ${pid}, ${m.model})\n`); },
      ...
      onTimeout(_, secs) { process.stderr.write(`\n⏰ Timeout (${secs}s)\n`); process.exitCode = 124; },
    },
  });
  const onSigint = () => { process.stderr.write("\n🛑 Interrupted\n"); process.exitCode = 130; session.cancel(); };
  process.once("SIGINT", onSigint);
  const ok = session.start();
  if (!ok) { process.exitCode = 1; session.dispose(); process.removeListener("SIGINT", onSigint); return; }
  process.stderr.write(`\n🏛️  Council running (${models.length} models, run: ${runId})\n\n`);
  try { await session.wait(); } finally { session.dispose(); process.removeListener("SIGINT", onSigint); }
...
function doSpawn(prompt: string, opts: { models?: string[]; cwd?: string }): void {
  ...
  for (const model of models) {
    try {
      ...
      for (const m of models) { try { fs.writeFileSync(agentPaths(runDir, m.id).done, "1", { flag: "wx" }); } catch {} }
      throw err;
    }
  }
...
function status(runId?: string): void {
  ...
  if (!meta) { process.stderr.write(`No run found: ${resolved}\n`); process.exitCode = 1; return; }
  ...
  for (const w of states) {
    ...
    if (w.status === "done") { doneCount++; process.stderr.write(`  ${green("✅")} ${bold(w.id.padEnd(8))} done${tc}\n`); }
    else if (w.status === "failed") { doneCount++; failedCount++; process.stderr.write(`  ${red("❌")} ${bold(w.id.padEnd(8))} failed: ${w.errorMessage ?? "unknown"}\n`); }
    else { process.stderr.write(`  ${yellow("⏳")} ${bold(w.id.padEnd(8))} running${tc}\n`); }
  }
...
async function printResults(runId?: string, wait = true): Promise<void> {
  ...
  if (wait) {
    await new Promise<void>((resolve) => {
      const check = () => {
        try { fs.accessSync(runDir); } catch { process.stderr.write("Run deleted.\n"); process.exitCode = 1; resolve(); return; }
        if (meta.agents.every(a => isAgentDone(runDir, a))) { clearInterval(timer); resolve(); }
      };
      const onSig = () => { process.exitCode = 130; clearInterval(timer); resolve(); };
  ...
  for (const w of states) {
    ...
    if (w.status === "done") { succeeded++; process.stdout.write((w.finalText || "(no text)") + "\n"); }
    else { failed++; process.stdout.write((w.finalText ? w.finalText + "\n" : "") + `ERROR: ${w.errorMessage ?? "empty output"}\n`); }
    if (w.usage.cost > 0) process.stdout.write(dim(`  cost: $${w.usage.cost.toFixed(4)} | tokens: ↑${w.usage.input} ↓${w.usage.output}`) + "\n");
    process.stdout.write("\n");
  }
  ...
  if (!fs.existsSync(path.join(runDir, "results.json"))) {
    const json = JSON.stringify({ runId: resolved, prompt: meta.prompt, completedAt: Date.now(),
      workers: states.map(w => ({ id: w.id, provider: w.provider, model: w.model, status: w.status, finalText: w.finalText, errorMessage: w.errorMessage, usage: w.usage }))
    }, null, 2);
    let md = `# pi-council results\nRun: ${resolved}\n\nQuestion:\n${meta.prompt}\n\n---\n\n`;
    for (const w of states) { md += `## ${w.id} — ${w.provider}/${w.model}\n\n${w.finalText || `(no output: ${w.errorMessage ?? "unknown"})`}\n\n---\n\n`; }
    try { fs.writeFileSync(path.join(runDir, "results.json"), json); fs.writeFileSync(path.join(runDir, "results.md"), md); } catch {}
  }
...
async function watch(runId?: string): Promise<void> {
  ...
  printDone();
  if (remaining.size === 0) { process.stderr.write(dim("All agents already finished.\n")); return; }
  process.stderr.write(dim(`Watching ${remaining.size} remaining agent(s)...\n\n`));
  await new Promise<void>((resolve) => {
    const done = () => { clearInterval(timer); process.removeListener("SIGINT", onSig); resolve(); };
    const onSig = () => { process.exitCode = 130; done(); };
    process.once("SIGINT", onSig);
    const timer = setInterval(() => { printDone(); if (remaining.size === 0) done(); }, 2000);
  });
}
...
function cancel(runId?: string): void {
  ...
  for (const agent of meta.agents) {
    ...
    try { const pid = parseInt(fs.readFileSync(paths.pid, "utf-8").trim(), 10); if (Number.isFinite(pid)) killPid(pid); } catch {}
...
function cleanup(runId?: string): void {
  ...
  if (!meta) { process.stderr.write(`No run found: ${resolved}\n`); process.exitCode = 1; return; }
  for (const agent of meta.agents) {
    try { const pid = parseInt(fs.readFileSync(agentPaths(runDir, agent.id).pid, "utf-8").trim(), 10); if (Number.isFinite(pid)) killPid(pid); } catch {}
  }
  try { fs.rmSync(runDir, { recursive: true, force: true }); } catch {}
  try {
    const latest = fs.readFileSync(getLatestFile(), "utf-8").trim();
    if (latest === resolved) {
      const dirs = fs.readdirSync(getRunsDir()).filter(d => d !== resolved).sort().reverse();
      if (dirs.length > 0) fs.writeFileSync(getLatestFile(), dirs[0]); else fs.unlinkSync(getLatestFile());
    }
  } catch {}
...
function list(): void {
  ...
  if (!fs.existsSync(runsDir)) { process.stderr.write("No runs found.\n"); return; }
  const dirs = fs.readdirSync(runsDir).sort().reverse();
  if (dirs.length === 0) { process.stderr.write("No runs found.\n"); return; }
  let latest = ""; try { latest = fs.readFileSync(getLatestFile(), "utf-8").trim(); } catch {}
  ...
  for (const dir of dirs) {
    ...
    try {
      ...
      statusStr = d === total ? (f > 0 ? yellow(`${total - f}/${total} ok`.padEnd(10)) : green("done".padEnd(10))) : yellow(`${d}/${total}`.padEnd(10));
    }
    process.stderr.write(`${dim(dir)}${dir === latest ? " ←" : ""}  ${statusStr} ${dim(`${total} models`.padEnd(12))} ${meta.prompt.replace(/\n/g, " ").slice(0, 40)}\n`);
  }
}
...
async function main(): Promise<void> {
  ...
  switch (command) {
    case "ask": if (!prompt) { process.stderr.write("Error: question required\n"); process.exitCode = 1; return; } await ask(prompt, { models, cwd, timeout }); break;
    case "spawn": if (!prompt) { process.stderr.write("Error: question required\n"); process.exitCode = 1; return; } doSpawn(prompt, { models, cwd }); break;
    case "status": status(runId); break;
    case "results": await printResults(runId); break;
    case "watch": await watch(runId); break;
    case "cancel": cancel(runId); break;
    case "cleanup": cleanup(runId); break;
    case "list": list(); break;
    case "help": printHelp(); break;
    case "version": { const p = new URL("../../package.json", import.meta.url); const pkg = JSON.parse(fs.readFileSync(p, "utf-8")); process.stdout.write(pkg.version + "\n"); break; }
    default: printHelp();
  }
}
