...
export interface WorkerState {
  id: string; provider: string; model: string; status: WorkerStatus;
  pid: number | null; toolCalls: number; events: number;
  preview: string; finalText: string; errorMessage: string | null;
  usage: { input: number; output: number; cacheRead: number; cacheWrite: number; cost: number };
}

export function loadMeta(runDir: string): RunMeta | null {
  try { return JSON.parse(fs.readFileSync(path.join(runDir, "meta.json"), "utf-8")); } catch { return null; }
}
...
function readPid(pidPath: string): number | null {
  try { const n = parseInt(fs.readFileSync(pidPath, "utf-8").trim(), 10); return Number.isFinite(n) ? n : null; } catch { return null; }
}
...
export function isAgentDone(runDir: string, model: ModelSpec): boolean {
  ...
  try { fs.accessSync(paths.done); return true; } catch {}
  const pid = readPid(paths.pid);
  if (pid === null) return false;
  if (!pidAlive(pid)) {
    try { fs.writeFileSync(paths.done, "", { flag: "wx" }); } catch {}
    return true;
  }
  return false;
}
...
export function refreshWorker(runDir: string, model: ModelSpec): WorkerState {
  ...
  if (isDone || (!isAlive && pid !== null)) {
    try { exitCode = fs.readFileSync(paths.done, "utf-8").trim(); } catch {}
  }
  ...
  const errText = (() => { try { return fs.readFileSync(paths.err, "utf-8").trim(); } catch { return ""; } })();

  return {
    id: model.id, provider: model.provider, model: model.model, status, pid,
    toolCalls: parsed.toolCalls, events: parsed.events,
